const spec = {
  ...
  info: {
    ...
    version: "1.0.0",
    description:
      "Backend API for the HarvestLynk agri-marketplace platform. Supports farmer and buyer flows including authentication, listings, orders, wallet, scans, and notifications.",
  },
  ...
  components: {
    securitySchemes: {
      ...
      bearerAuth: {
        ...
        bearerFormat: "JWT",
        description:
          "Bearer token — same JWT value, for use in tests or API clients",
      },
    },
    schemas: {
      Error: {
  ...
  paths: {
    ...
    "/api/v1/orders/{id}/status": {
      patch: {
        tags: ["Orders"],
        summary:
          "Farmer advances order status (payment_confirmed → processing → ready_for_pickup)",
        parameters: [
          {
            name: "id",
            in: "path",
    ...
    "/api/v1/orders/{id}/cancel": {
      patch: {
        tags: ["Orders"],
        summary:
          "Cancel an order (buyer or farmer, only for cancellable statuses)",
        parameters: [
          {
            name: "id",
            in: "path",
